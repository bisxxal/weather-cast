...
return (
    <div className='w-full h-screen bg-[#295a99cf] flex flex-col text-white justify-between py-5 bg-[url("https://images.unsplash.com/photo-1675640145657-7ac2289cb4bb?q=80&w=1974&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D")] bg-center bg-cover bg-no-repeat'>

        <div className='px-5 mb-7 lg:mb-0  lg:px-9 items-center flex justify-between ' > 
        <div> <h1 className='text-[30px] lg:text-[4vh]  text-white '>wpCAST</h1></div>
        ...
        <div className='flex items-center bg-[#33619d44] border-[#ffffff84]  border-[0.1px] backdrop-blur-[9px] rounded-3xl px-3'> <input   onKeyDown={(e) => { 
                    ...
                    }}  onChange={(e)=>setinput()}   className= ' text-[#ffffff] text-[18px] lg:w-[400px] h-[40px] px-2  border-none outline-none bg-transparent ' type="text" placeholder='Search place name ' />
        ...
        <div className='temp lg:w-[330px] w-full border-[#ffffff50]  border-[0.1px] backdrop-blur-[9px] px-[10px] lg:px-[20px] lg:pr-[35px] rounded-2xl py-4 bg-[#33619d4e] h-[200px]  lg:h-[300px]'> 
         
           <p>Current Weather </p> 
           
        <div className='flex items-center  gap-5 justify-evenly pt-5'>
             <img className='w-[100px] lg:w-[140px] ' src={wicon} alt="" /> 
        <div className=' relative'> <h1 className='text-[60px] font-semibold'>{data?Math.floor(data.main.temp):'0'} </h1>
         <p className=' absolute text-[60px] top-[-50px] right-[-30px]'> &deg;c</p>
          </div> 
           </div> 
        <p className=' text-[25px] lg:mt-5 text-center'>{data?data.weather[0].description:'Loading'}</p>
        </div>
        ...
        <div className='flex lg:gap-[unset] gap-4  w-full justify-evenly flex-wrap '>
            <div className='flex  flex-col gap-4 items-center justify-center  h-[150px] lg:h-[240px] w-[45%] lg:w-[300px] border-[#ffffff4b] rounded-2xl border-[0.1px] backdrop-blur-[20px]'> <p>Wind Speed</p> <div className='flex items-center gap-4'><BsWind className=' text-[30px]' /> <p className=' text-[25px] font-medium'>{data?data.wind.speed:'0'}</p>km/h </div></div>
             <div className='flex flex-col gap-4  items-center justify-center h-[150px] lg:h-[240px] w-[45%] lg:w-[300px] border-[#ffffff4b] rounded-2xl border-[0.1px] backdrop-blur-[20px]'> <p>Visibility</p> <div className="flex items-center gap-4"><MdOutlineVisibility  className=' text-[30px]' /> <p className=' text-[25px] font-medium'>{data?data.visibility:'0'}</p></div>   </div>
